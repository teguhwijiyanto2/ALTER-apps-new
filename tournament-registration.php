...
function validate_1() {
	...
	if(document.getElementById("input-logo").value == "") {
		document.getElementById("input-logoError").style.display = "block";
		is_error_exist = "Y";
	}
	else {
		document.getElementById("input-logoError").style.display = "none";
	}

	if(document.getElementById("team_name").value.trim() == "") {
		document.getElementById("team_nameError").style.display = "block";
		is_error_exist = "Y";
	}
	else {
		document.getElementById("team_nameError").style.display = "none";
	}

	if(document.getElementById("input_response").value === "-") {
		document.getElementById("team_nameError").innerHTML = "Team Name is required and must still available!";
		document.getElementById("team_nameError").style.display = "block";
		is_error_exist = "Y";
	}
	else {
	...
	var xplayers_per_team = document.getElementById("xplayers_per_team").value;
	for(i=1; i<=xplayers_per_team; i++) {
		
		if(document.getElementById("players_username_"+i).value == "") {
			document.getElementById("players_usernameError_"+i).style.display = "block";
			is_error_exist = "Y";
		}
		else {
			document.getElementById("players_usernameError_"+i).style.display = "none";
		}
		
		if(document.getElementById("players_id_"+i).value == "") {
			document.getElementById("players_idError_"+i).style.display = "block";
			is_error_exist = "Y";
		}
		else {
			document.getElementById("players_idError_"+i).style.display = "none";
		}
	...
	if(is_error_exist == "N") {
        ...
        <!-- Add New Team Start -->
        <form action='tournament-registration-process.php' method='POST' enctype='multipart/form-data' id="myForm">
		<input type='hidden' name='tournament_idx' id='tournament_idx' value='<?php echo $_POST['tournament_idx']; ?>'>
		<input type='hidden' name='tournament_codex' id='tournament_codex' value='<?php echo $_POST['tournament_codex']; ?>'>
		<input type='hidden' name='xplayers_per_team' id='xplayers_per_team' value='<?php echo $_POST['xplayers_per_team']; ?>'>		
		...
		<input type='hidden' name='xparticipant_fee' value='<?php echo $results_B['participant_fee']; ?>'>
          ...
          <div class="p-3 bg-dark rounded-3 mt-3">
			...
			<div class="text-error" id="input-logoError" style="display: none; color: red;">Team Logo is required!</div>
            ...
            >
              ...
              <div class="form__group-input mt-2">
                <input type="text" name="team_name" id="team_name" placeholder="Alter Team" onkeyup="checkTeamName(this.value,document.getElementById('tournament_codex').value)" />
				<!--<i class="bi bi-check-circle-fill fs-3 text-success"></i>-->
                <div id="span_response">&nbsp;</div>
				<span style="display: none;"><input type="text" id="input_response"></span>				
				
				<script>
				function checkTeamName(str,tournament_codex) {
				  var xhttp;
				  if (str.length == 0) { 
				  ...
				  xhttp.open("GET", "checkTeamName.php?q="+str+"&tournament_codex="+tournament_codex+"", true);
				  xhttp.send();   
				}
				</script>

              </div>
			  <div class="text-error" id="team_nameError" style="display: none; color: red;">Team Name is required!</div>
            ...
            <div class="mt-1">
              ...
              <div class='mt-2'>
                ...
                <div class='form__group-input mt-2'>
                  <input type='text' name='players_username[$x]' id='players_username_".$x."' placeholder='Player ".$x."&#39;s &nbsp; Name' />				  
                </div>
				<div class='text-error' id='players_usernameError_".$x."' style='display: none; color: red;'>Player ".$x."&#39;s &nbsp; Name is required!</div>
				
                <div class='form__group-input mt-2'>
                  <input type='text' name='players_id[$x]' id='players_id_".$x."' placeholder='Player ".$x."&#39;s &nbsp; ".$array_games[$results_B['game_name_id']]." ID' />
                </div>
				<div class='text-error' id='players_idError_".$x."' style='display: none; color: red;'>Player ".$x."&#39;s &nbsp; ".$array_games[$results_B['game_name_id']]." ID is required!</div>
          ...
          <div id="span_submit">
			  <!-- <input type='submit' class='btn btn-outline-light rounded-pill my-4 w-100' value='Submit'> -->
