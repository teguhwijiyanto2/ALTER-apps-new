...
<html lang="en">
  ...
  <body>
    <form action="play-process.php" method="POST" id="myForm">
	<input type="hidden" name="user_id_profile" value="<?php echo $user_id_profile; ?>">	
...
function validate_1() {
	...
	if(document.getElementById("selGame").value == "") {
		document.getElementById("selGameError").style.display = "block";
		is_error_exist = "Y";
	}
	else {
		document.getElementById("selGameError").style.display = "none";
	}

	if(document.getElementById("datetime_play").value == "") {
		document.getElementById("datetime_playError").style.display = "block";
		is_error_exist = "Y";
	}
	else {
		document.getElementById("datetime_playError").style.display = "none";
	}
	
	if(document.getElementById("selHours").value == "") {
		document.getElementById("selHoursError").style.display = "block";
		is_error_exist = "Y";
	}
	else {
		document.getElementById("selHoursError").style.display = "none";
	}	
        ...
        <div class="container px-4">
          ...
          <div class="p-3 bg-dark rounded-3">
            ...
            >
                ...
                <select name="selGame" id="selGame" required
			...
			<div class="text-error" id="selGameError" style="display: none; color: red;">Game is required!</div>				
            </div>
						
			<div class="form__group mt-3">
              <label>Date & Time</label>
              <div class="form__group-input">
                <input required type="datetime-local" class="form-control" name="datetime_play" id="datetime_play"  onchange="document.getElementById('div_TxtDateTimePlay').innerHTML=this.value; document.getElementById('div_TxtDateTimePlay2').innerHTML=this.value;" />
                <script>
					var today = new Date().toISOString().slice(0, 16);
					document.getElementById("datetime_play").min = today;
                </script>
			  </div>
			<div class="text-error" id="datetime_playError" style="display: none; color: red;">Date & Time is required!</div>
            </div>		
            ...
            >
                ...
                <select name="selHours" id="selHours" onchange="duration(this.value)" required
                  class="form-select form__group-select"
			...
			<div class="text-error" id="selHoursError" style="display: none; color: red;">Duration of Play (in Session) is required!</div>	
            </div>
        ...
        <div class="container px-4">
          ...
          <div class="p-3 bg-dark rounded-3">
            ...
            <div class="mt-3 d-flex flex-column gap-1">
              ...
              >
                <span>Payment</span>
                <span class="text-light fs-5"><?= ($option && $option['fee']) ? number_format($option['fee']) : 'Free' ?></span>
              </div>
            </div>
          </div>
          <!-- Play With End -->
          ...
          <div class="p-3 bg-dark rounded-3 mt-3">
            ...
            <div class="mt-3 d-flex flex-column gap-1">
              <div
                class="d-flex flex-row align-items-center justify-content-between text-secondary"
              >
                <span><b id="div_TxtSelectedHour3"></b> x session fee</span>
