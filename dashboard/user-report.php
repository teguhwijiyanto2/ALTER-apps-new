...
$no = 0;
$results_2 = DB::query("
	select *, DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), birthdate)), '%Y') + 0 AS age from users 			 	
");
foreach ($results_2 as $row_2) {
	$option = DB::queryFirstRow("SELECT * FROM matchmaking_option WHERE user_id = %i",$row_2['id']);
