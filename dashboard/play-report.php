							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title">Play Report </h4>
									</div>
								</div>
								<div class="card-body">
									<!-- Modal -->
									...
									<div class="table-responsive">
										<table id="add-row" class="display table table-striped table-hover" >
											...
											<!-- "- Requestor Name
											- Talent / User yang direquest
											- Game
											- Date (Bisa di filter)
											- Session
											- Fee
											- Total Fee
											- Status (Bisa di filter)" -->
											<!--
											<tfoot>
												<tr>
													<th>No</th>
...
$no = 0;
$results_2 = DB::query("
	select * from matchmaking_availability 			 	
");
foreach ($results_2 as $row_2) {
	$request_profile = DB::queryFirstRow("SELECT * FROM users where id=%i", $row_2['requestor_id']);
	$talent_profile = DB::queryFirstRow("SELECT * FROM users where id=%i", $row_2['approver_id']);
	$request = $request_profile ? $request_profile['name'] : '-';
	$talent = $talent_profile ? $talent_profile['name'] : '-';
	...
	echo "
												<tr>
													...
													<td>".$row_2['date_time']."</td>
													<td>".$row_2['num_of_hours']."</td>
													<td>".$row_2['unit_price']."</td>
													<td>".$row_2['amount']."</td>
													<td>".$row_2['request_status']."</td>
												</tr>
	";
} // foreach ($results_2 as $row_2) {											
?>												
